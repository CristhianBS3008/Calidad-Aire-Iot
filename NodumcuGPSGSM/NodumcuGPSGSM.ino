...
char url[ ]="api.thingspeak.com/update?api_key=ACSLVHRPLC6XKOY2&field5=";   //CAMBIAR API_KEY
char url2[]="&field6=";
...
void HTTPRequest()
{
  ...
  char sensorValue2[50];
  dtostrf(longitud.toFloat(),5,6,sensorValue2);
  Serial.println(sensorValue2);
  snprintf(aux_str, sizeof(aux_str), "AT+HTTPPARA=\"URL\",\"%s\%s\%s\%s\"\r\n", url, sensorValue,url2, sensorValue2);
  sendATcommand(aux_str, "OK\r\n", TIMEOUT);
  delay(3000);
  attempts = 0;//tries 3 times or gets on the loop until sendATcommand != 0
